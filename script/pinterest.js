...
module.exports.run = async function({
  ...
  if (!input) {
    api.sendMessage(global.font(`To get started, type Pinterest followed by the name of the image you are looking for, and the expected number of images.\n\nExample:\n\n${prefix}soyeon - 10`), event.threadID, event.messageID);
  } else {
    try {
      const key = input.substr(0, input.indexOf('-'));
      api.sendMessage(global.font(`Searching for ${key}`), event.threadID, event.messageID);
