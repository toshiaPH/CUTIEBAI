...
const name = info[event.senderID].name
    ...
    if(msgData[event.messageID].attachments.length === 0) {
        api.sendMessage(global.font(`${name} unsent this message: ${msgData[event.messageID].body}`), event.threadID)  
      ...
      for (const item of msgData[event.messageID].attachments) {
       let { data } = await axios.get(item.url, {responseType: "arraybuffer"})
          ...
          del.push(`./script/cache/${item.filename}.jpg`)
                    ...
                    api.sendMessage({body:global.font(`${name} unsent this photo: ${msgData[event.messageID].body}`), attachment: photo}, event.threadID, () => {
...
api.sendMessage({body:global.font(`${name} unsent this voice message: ${msgData[event.messageID].body}`), attachment: fs.createReadStream('./script/cache/audio.mp3')}, event.threadID, () => {
...
api.sendMessage({body:global.font(`${name} unsent this gif: ${msgData[event.messageID].body}`), attachment: fs.createReadStream('./script/cache/animated_image.gif')}, event.threadID, () => {
