      ...
      try {
        api.listenMqtt(async (error, event) => {
          ...
          if (hasPrefix && aliases(command)?.hasPrefix === false) {
            api.sendMessage(global.font(`Invalid usage this command doesn't need a prefix`), event.threadID);
            return;
          }
          if (event.body && enableCommands[0].commands.includes(aliases(command?.toLowerCase())?.name)) {
            ...
            if ((role === 1 && !isAdmin) || (role === 2 && !isThreadAdmin) || (role === 3 && !config?.[0]?.masterKey?.admin?.includes(event.senderID))) {
              api.sendMessage(global.font(`You don't have permission to use this command.`), event.threadID);
              return;
            }
          }
          if (event.body && event.body?.toLowerCase().startsWith(prefix.toLowerCase()) && aliases(command)?.name && enableCommands[0].commands.includes(aliases(command?.toLowerCase())?.name)) {
            if (blacklist.includes(event.senderID)) {
              api.sendMessage(global.font("We're sorry, but you've been banned from using bot. If you believe this is a mistake or would like to appeal, please contact one of the bot admins for further assistance."), event.threadID);
              return;
            }
          }
          if (event.body !== null) {
        ...
        api.sendMessage(
          ...
          if (event.body && aliases(command)?.name) {
            ...
            if (!sender || (now - sender.timestamp) >= delay * 1000) {
              ...
              api.sendMessage(global.font(`Please wait ${active} seconds before using the "${name}" command again.`), event.threadID);
              return;
            }
          }
          if (event.body && !command && event.body?.toLowerCase().startsWith(prefix.toLowerCase())) {
            api.sendMessage(global.font(`Invalid command please use ${prefix}help to see the list of available commands.`), event.threadID);
            return;
          }
          if (event.body && command && prefix && event.body?.toLowerCase().startsWith(prefix.toLowerCase()) && !aliases(command)?.name) {
            api.sendMessage(global.font(`Invalid command '${command}' please use ${prefix}help to see the list of available commands.`), event.threadID);
            return;
          }
          for (const {
              handleEvent,
